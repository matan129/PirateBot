...
set maxturns=%4
if [%4]==[]  set maxturns=1000
set points=%5
if [%5]==[]  set points=1000
...
@echo starting...
%pyexe% "%~dp0lib\playgame.py" --loadtime 10000 -e -E -d --debug_in_replay --engine_seed 42 --player_seed 42 --log_dir "%~dp0lib\game_logs" --map_file "%map%" "%bot1%" "%bot2%" --turns %maxturns% --maxpoints %points%
goto:EOF

:usage
@echo Usage: %0 ^<Player One Bot^> ^<Player Two Bot^> [Map] [maxturns] [maxpoints]
exit /B 1
