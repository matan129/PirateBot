...
function colorize(img, colors) {
	...
	canvas.width = img.width * colors.length;
	...
	for (i = 0; i < colors.length; i++) {
		...
		if (c) {
			if (typeof c == 'string') {
				c = [
					parseInt(c.charAt(2), 16) + 16 * parseInt(c.charAt(1), 16),
					parseInt(c.charAt(2), 16) + 16 * parseInt(c.charAt(3), 16),
					parseInt(c.charAt(6), 16) + 16 * parseInt(c.charAt(5), 16)
				];
			}
