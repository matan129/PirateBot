...
namespace Britbot
{
  public class Group
  {
    ...
    public void Move()
    {
      ...
      for (int index = 0; index < this.Pirates.Count; ++index)
      {
        Pirate myPirate = Bot.Game.GetMyPirate(this.Pirates[index]);
        Bot.Game.SetSail(myPirate, Enumerable.First<Direction>((IEnumerable<Direction>) Bot.Game.GetDirections(myPirate, this.Target.GetLocation())));
      }
    }
      ...
      if (direction <= 110)
      {
        if (direction != 101)
        {
          if (direction == 110)
          {
            ...
            int num = location.get_Row() - 1;
            location.set_Row(num);
          }
        }
        else
        {
          Location location = cur;
          int num = location.get_Col() + 1;
          location.set_Col(num);
        }
      }
    ...
    private List<Location> GetMLocList(Location loc)
    {
      ...
      for (int index1 = -1; index1 <= 1; ++index1)
      {
        for (int index2 = -1; index2 <= 1; ++index2)
          list.Add(new Location(loc.get_Row() + index1, loc.get_Col() + index2));
      }
      return list;
    }
  }
